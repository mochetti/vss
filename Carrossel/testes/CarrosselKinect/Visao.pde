...
//          display.pixels[pix + 640] = color(255);
//        } else {
//          display.pixels[pix] = img.pixels[offset];
//          display.pixels[pix + 640] = color(0);
//        }
//      }
//    }
...
// Funcao que encontra saliencias
void saliencias() {
  ...
  // What we'll show the user
  PImage displayDepth = createImage(kinect.width, kinect.height, ALPHA);
  PImage displayRGB = createImage(kinect.width, kinect.height, RGB);

  // Being overly cautious here
  if (depth == null || kinectDepth == null) return;
  
  displayDepth.loadPixels();
  ...
  for (int x = 0; x < kinect.width; x++) {
    for (int y = 0; y < kinect.height; y++) {
      ...
      // Raw depth
      int rawDepth = depth[offset];
      int pix = x + y * kinect.width;
      boolean dentro = x > comeco.x && x < fim.x && y > comeco.y && y < fim.y;
      if (rawDepth < depthThreshold && rawDepth > depthThreshold - alturaDepth && dentro) {
        // A red color instead
        displayDepth.pixels[pix] = color(150, 50, 50);
      } else {
        displayDepth.pixels[pix] = kinectDepth.pixels[offset];
      }
    }
  }
  
  displayDepth.updatePixels();
  displayRGB.loadPixels();
  ...
  for (int x = 0; x < kinect.width; x++) {
    for (int y = 0; y < kinect.height; y++) {
      ...
      if (rawDepth < depthThreshold && rawDepth > depthThreshold - alturaDepth && dentro) {
        // Cor real
        displayRGB.pixels[pix] = kinectRGB.pixels[offset];
      } else {
        displayRGB.pixels[pix] = color(0);
      }
    }
  }
  displayRGB.updatePixels();

  image(displayRGB, 640, 0);
  image(displayDepth, 0, 0);
}
...
boolean track() {
  ...
  if(oldBlobs.size() > 0){
    for(Blob b : oldBlobs) {
      ...
      if(xi < comeco.x) xi = int(comeco.x);
      int xf = prevX + raioBusca;
      if(xf > fim.x) xf = int(fim.x);
      int yi = prevY - raioBusca;
      if(yi < comeco.y) yi = int(comeco.y);
      int yf = prevY + raioBusca;
      if(yf > fim.y) yf = int(fim.y);
...
boolean search (int xi, int xf, int yi, int yf, Blob b) {
  ...
  for (int x = xi; x < xf; x++ ) {
    for (int y = yi; y < yf; y++ ) {
      int loc = x + y * kinectRGB.width;
      // What is current color
      color currentColor = kinectRGB.pixels[loc];
...
void searchNew (int c) {
  ...
  for (int x = int(comeco.x) ; x < fim.x; x++ ) {
    for (int y = int(comeco.y); y < fim.y; y++ ) {
      int loc = x + y * kinectRGB.width;
      // What is current color
      color currentColor = kinectRGB.pixels[loc];
...
boolean ordenar() {
  int maiorId = 0;
  for(Blob b : blobs) if(b.id > maiorId) maiorId = b.id;
  Blob newBlobs[] = new Blob[maiorId + 1];
